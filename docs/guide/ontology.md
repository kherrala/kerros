...
virtual-boundary geometry describes their shape. Host applications add access rules, bookings and
observations using the same entity IDs; those policies and events are not part of the topology.
...
Consider an office with a lobby opening into a cafe, a finance department containing a server room,
and a lift serving two floors. Model the lobby, cafe and rooms as spaces. Finance and Server room
are semantic zones, with Server room nested under Finance. A shared virtual boundary connects the
lobby and cafe without inventing a door. The server-room opening supplies its portal.

Create the lift as an `elevator` object with the served floors and physical footprint. A circulation
zone can group its shaft or landing spaces, but a zone alone does not create an operable car.
Connect landings to their floor's circulation. See [lifts and stairs](#lifts-and-stairs).

Finance membership says which spaces belong together. It does not authorize entry to the server
room. The [application guides](/applications/) develop access control, reservations, monitoring and
wayfinding on top of these common facts.
...
```ts
import { addZone, nestZone, transact, type ProjectDocument } from '@kerros/schema';

export function groupFinance(project: ProjectDocument, officeId: string, serverId: string) {
  return transact(project, draft => {
    const server = addZone(draft, 'Server room', [serverId], 'security');
    const finance = addZone(draft, 'Finance', [officeId], 'security');
    if (!nestZone(draft, finance.id, server.id)) throw new Error('Invalid zone nesting.');
  });
}
```

Here the IDs must identify existing spaces. `addZone` filters out non-space IDs rather than
throwing. Validate required destinations explicitly when silent filtering would be inappropriate;
the [visitor guide](/applications/visitor-management#create-a-visit-area) shows how.
...
Use explicit endpoint IDs when mapping directional crossings into a host system. The
[access-control example](/applications/access-control#project-directional-crossings) handles both
one-way passages and adjacent-zone projections.
...
establish arrival. Occupancy and anti-passback calculations require actual host events and their
uncertainty; changing `attests` does not create that evidence.
...
references, but does not certify access policy, physical accessibility or source-drawing accuracy.

Merging spaces removes an identity. Choose the survivor and reconcile host bindings, bookings and
...
members rideable. For renderable, navigable transport and playback, follow
[indoor wayfinding](/applications/wayfinding#floors-stairs-and-escalators).
...
connect the car/landing spaces to the relevant lobbies. Host access rules may distinguish those
landing destinations even when they share a car. The host's `elevators` adapter supplies passenger
commands and live car state.
...
The schema describes geometry, containment, membership and connectivity. The host owns permissions,
people, bookings, events and device operations. This separation lets the same portable document
support the different [application guides](/applications/) without a separate floor plan for each.
