...
test('IndexedDB keeps legacy saves readable and stores the largest generated office sample', async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    const persistencePath = '/src/adapters/persistence.ts', factoryPath = '/src/model/factory.ts', demoPath = '/app/demo/backrooms.ts';
    ...
    const previous = new LocalProjectRepository(), projects = new IndexedProjectRepository(previous);
    ...
    const names = (await projects.list()).filter((p: { id: string }) => p.id === old.id).map((p: { name: string }) => p.name);
    ...
    try { await projects.save(large); } finally { Storage.prototype.setItem = setItem; }
    const loaded = await new IndexedProjectRepository(previous).load(large.id);
    await projects.delete(old.id);
    return { legacyName, names, deleted: await projects.load(old.id), legacyDeleted: await previous.load(old.id),
