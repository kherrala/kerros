...
test('bath music renders a finite stereo phrase with a quiet entrance and room tail', async ({ page }, info) => {
  await page.goto('/app.html');
  const audio = await page.evaluate(async () => {
    const path = '/src/map/bathMusic.ts';
    const { bathMusic } = await import(path);
    const ctx = new OfflineAudioContext(2, 28 * 22050, 22050);
    const volume = ctx.createGain();
    volume.gain.value = 0.55;
    volume.connect(ctx.destination);
    const music = bathMusic(ctx, volume);
    ...
    for (let i = 0; i < left.length; i++) {
      ...
      if (i > 23 * 22050) tail += left[i] ** 2 + right[i] ** 2;
    }
  ...
  await writeFile(info.outputPath('bath-music.wav'), Buffer.from(audio.wav, 'base64'));
});
